---
traits: [analogico:Analogica, "versatile [P]"]
source: Player Core (Eng), pag. 264
original-name: Dueling Sword
original-srd: https://2e.aonsrd.com/equipment/weapons/13-dueling-sword
itemCategories: [base-weapon]
navigation-disable: true
title: Spada da Duello
level: "0+"
itemWeapon:
  price: 10
  dice: 1
  upgrades: 1
#[[weaponSubItems Spada da Duello, Dueling Sword]]
---

# {{% get `.params.title` %}} - Oggetto {{% get `.params.level` %}}

**Prezzo** {{% get `.params.itemWeapon.price | lang.FormatNumber 0` %}} crediti;
**Danni** {{% get `.params.itemWeapon.dice` %}}d8 T; **Volume** 1

**Mani** 1

**Tipo** Mischia; **Categoria** Da Guerra; **Gruppo**
[Spada](/gruppi-armi/spada)

**Migliorie** {{% get `.params.itemWeapon.upgrades` %}}

---

Disponibili in una varietà di stili, queste armi tradizionali e collaudate sono
state perfezionate con tecniche di forgiatura avanzate e materiali moderni.

#[[weaponUpgradeTable Spada da Duello, spada-da-duello]]
