---
traits: [tecnologico:Tecnologica]
source: Player Core (Eng), pag. 265
original-name: Laser Pistol
original-srd: https://2e.aonsrd.com/equipment/weapons/40-laser-pistol
itemCategories: [base-weapon]
navigation-disable: true
title: Pistola Laser
level: "0+"
itemWeapon:
  price: 30
  dice: 1
  upgrades: 1
#[[weaponSubItems Pistola Laser, Laser Pistol]]
---

# {{% get `.params.title` %}} - Oggetto {{% get `.params.level` %}}

**Prezzo** {{% get `.params.itemWeapon.price | lang.FormatNumber 0` %}} crediti;
**Danni** {{% get `.params.itemWeapon.dice` %}}d6 F; **Volume** 1

**Mani** 1; **Gittata** 12 m.; **Ricarica** 1

**Tipo** Distanza; **Categoria** Semplice; **Gruppo**
[Laser](/gruppi-armi/laser)

**Migliorie** {{% get `.params.itemWeapon.upgrades` %}}; **Caricatore** 10
[cariche](/equipaggiamento/batteria); **Dispendio** 2

---

Questa pistola compatta è l’arma da fianco più diffusa nei Mondi del Patto
grazie alla sua affidabilità, resistenza e convenienza. Le pistole laser più
comuni hanno un corpo leggero in policarbonato che termina in una canna lunga e
sottile.

#[[weaponUpgradeTable Pistola Laser, pistola-laser]]
